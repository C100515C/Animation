...
- (void)startAnimationHeartLayer:(CALayer *)heartLayer
{
    ...
    CGPoint beginPoint = CGPointMake(heartLayer.position.x, heartLayer.position.y);
    CGMutablePathRef customPath = CGPathCreateMutable();
    CGPathMoveToPoint(customPath, NULL, beginPoint.x, beginPoint.y);
    CGPathAddQuadCurveToPoint(customPath, NULL, 140,400, 300, 200);
    CGPathAddQuadCurveToPoint(customPath, NULL, 300, 300, destination.x, destination.y);
    
    pathAnimation.path = customPath;
    
    //透明度动画
    CAKeyframeAnimation *alphaAnimation =
    [CAKeyframeAnimation animationWithKeyPath:@"opacity"];
    alphaAnimation.values = @[@1.0,@0.7,@0.0];
    alphaAnimation.keyTimes = @[@0.0,@0.80,@1.0];
    ...
    CATransform3DMakeRotation(M_PI/4, 1.0f, 3.0f, (arc4random() % 2000) /1000.0f -1);
    ...
    [groupAnimation setAnimations:@[posAnimation,alphaAnimation,angleAnimation,scaleAnimation,pathAnimation]];
